...
- Dialogue 执行面板链路已经闭合到 task-event wrapper：选中 task 后能看到 worker/status/最近输出/执行时间线，并通过 task SSE 事件缩短刷新延迟；这不是 token 级 stdout streaming。

仍未闭合或不能夸大的边界：

- `app_server_json_rpc` 和 `mcp` 还不是通用 dynamic provider protocol；Codex app-server 仍是内置 Codex 主链，不应把 `providers.yaml` 示例理解成所有 app-server provider 都已动态化。当前 discovery 只把这类配置记录为 unsupported provider，不注册 runnable worker。
