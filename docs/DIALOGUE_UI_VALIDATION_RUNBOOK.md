...
- 当前 route box plan 已把 provider recovery 避让提升为 `primaryRecoveryNote`，首屏直接渲染“恢复阶段会优先避开 <provider>”与人话原因；drawer 只保留补充 chips / candidate / timeline，summary 显示 `展开路由细节 / 展开路由轨迹`，不再裸露 `route / timeline` 混合标签。
...
- façade 回执 badge 也应中文化：`task_receipt / task_progress / task_result / worker_round` 分别显示 `最新回执 / 最新进展 / 最新结果 / 最新回合`，不要回退到 `latest receipt / latest progress / latest result / latest round`。
...
- Provider run 排障面板与 worker round 展开区已将可见文件/诊断标签收口为中文：`运行目录`、`最后输出`、`事件日志`、`标准输出`、`运行元数据`、`Provider 诊断`、`Provider 运行文件`；底层 `kind=last_message|events|stdout|metadata|prompt` API 合同不变。Dialogue / Console 点击 `事件日志`、`标准输出` 时优先用 `stream=true` 订阅尾部窗口，并带 `tail=true&max_lines=80`，避免长 JSONL / stdout 只看到文件头；其他文件仍按默认文件头 64 KiB 预览。
