...
2. 再做严格 UTF-8 解码
3. **新增**：如果 UTF-8 解码成功但结果看起来像乱码（包含大量替换字符 U+FFFD 或典型的 GBK 乱码模式），继续尝试 fallback 字符集
4. 如果严格 UTF-8 失败，再走第三方字符集探测
5. 探测仍不确定时：
   - Windows：优先尝试宿主机默认 charset，再尝试 `GB18030` / `GBK`
   - 非 Windows：回退宿主机默认 charset

**乱码检测机制**：通过分析解码结果中替换字符的比例、可打印 ASCII 字符的比例，以及是否包含典型的 GBK 乱码模式（Latin-1 补充区域字符）来判断是否为乱码。

这条策略的目标不是“什么都猜对”，而是：

1. **尽量用 UTF-8**
2. 当外部工具明确不是 UTF-8 时，不再把结果直接打成乱码
3. **新增**：即使 UTF-8 解码成功，如果结果看起来像乱码，仍会尝试其他字符集
...
2. provider/native worker 失败摘要应优先显示可读文本，而且应收成短摘要；不能把长 prompt 回显、目录 listing 或整段原始输出直接塞进 `failure_summary_readable`
