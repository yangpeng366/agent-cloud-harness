...
- 2026-07-29: E1.1 长任务收口合同深化落地。ControlNodeGraph.resolveAction 后新增 buildDecisionRationale，输出显式 decision_rationale（引用 goal progress done/blocked/open + execution action + completion status/alignment -> resolved action），经 withMetadataEntries + sameState 持久化到 task metadata（sameState 比较 metadata 故可落库）。DecisionRationaleCompositionTest 5 场景（反射覆盖 all-done/blocked/open/no-subgoals/mixed）。E1 验收 #1 done；#2（progress_summary 语义化）/ #3（LLM subgoal judgment 进 decide）deferred（#2 会破坏 ~10 条精确相等断言，拟改 additive progress_detail 字段避免回归）。decide/orchestration/goal focused 回归 79/0。
- 2026-07-29: E1 长任务收口合同进入可观测层。decision_rationale / progress_detail / progress_summary 已作为 JudgmentTraceView 一等字段经 TaskService.buildJudgmentTraceView 投影到 GET /tasks/{id}/judgment_trace；/live_flow 通过嵌入 judgment_trace 继承。JudgmentTraceView 为 record，仅 1 个 construction site，无破坏性；新增 TaskServiceLiveFlowViewTest.getJudgmentTraceSurfacesDecisionRationaleAndProgressDetail验证 metadata 字段透传。focused 回归 116/0/0/0（含 live_flow / packet / checkpoint / guard 合同不回归）。
- 2026-07-29: E1.2 长任务收口合同深化落地（additive progress_detail）。新增 progress_detail 语义字段：done/blocked/open 计数 + blocked subgoal 标题（形如 "1/3 done, 1 blocked, 1 open; blocked: API integration"），在 task 创建（ProviderTaskContractNormalizer.buildProgressDetail）与 subgoal 迁移（ControlNodeGraph.autoUpdateSubgoalStatus 两处 runtime 站点）与 progress_summary 同步生成。不改 progress_summary 以避免破坏 ~9 条精确相等断言（GoalProgressAutoUpdateTest / TaskServiceGoalContractTest）。ProgressDetailCompositionTest 6 场景（反射覆盖 all-done/blocked-with-title/in-progress-open/no-subgoals/mixed/长标题截断）。focused 回归 106/0（含 packet/checkpoint/guard 合同不回归）。E1 验收 #2 done。
