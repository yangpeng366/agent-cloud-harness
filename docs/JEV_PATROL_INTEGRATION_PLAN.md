...
| **Phase 6** | 脑洞 A/B/C（草案已存） | A：`docs/JEV_PATROL_PHASE6_CASCADE_DESIGN.md`（跨窗口 keep_probability 联动网）<br>B：`docs/JEV_PATROL_PHASE6_HEATMAP_DESIGN.md`（per-project 评分热力图）<br>C：项目健康度评分（合并到 B） | Phase 5 | 长期（默认 disabled，仅设计稿） |
